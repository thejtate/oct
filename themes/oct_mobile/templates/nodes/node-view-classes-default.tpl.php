...
<?php if ($node->view->name == 'classes'): ?>
  ...
  <?php foreach ($node->field_class_date_from as $k => $dateFrom): ?>
    <?php if (!empty($dateFrom['view'])): ?>
      ...
      <p><strong><?php print $weekdays; ?></strong></p>
      <p><strong><?php print $dateFrom['view']; ?> - <?php print $node->field_class_date_to[$k]['view']; ?></strong>
        <br />
        <strong><?php print $node->field_class_time_from[$k]['view']; ?> - <?php print $node->field_class_time_to[$k]['view']; ?></strong>
      </p>
      <p>
        <strong>
          <?php print t('from '); ?>
          ...
          <?php else: ?>
            <?php print t('and up'); ?>
          <?php endif; ?>
          $<?php print $node->field_class_price[0]['value']; ?>
        </strong>
      </p>
